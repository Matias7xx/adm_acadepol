...
class CertificadoHelper
{
     ...
     */
    public static function gerarCaminhoParaUsuario(User $user, string $nomeCurso, string $numeroCertificado): string
    {
        ...
        if (Storage::disk('local')->exists($caminhoCompleto)) {
            ...
            $nomeArquivo = 'certificado_' . $numeroCertificado . '_' . $timestamp . '.pdf';
            $caminhoCompleto = $diretorio . '/' . $nomeArquivo;
        }
    ...
    public static function gerarCaminhoUnico($matricula, string $numeroCertificado = null): string
    {
        // Se é um usuário direto
        if ($matricula instanceof User) {
            throw new \InvalidArgumentException('Use gerarCaminhoParaUsuario() para usuários diretos');
        }
        ...
        try {
            ...
            if (Storage::disk('local')->exists($diretorio)) {
                ...
                if (empty($arquivos)) {
                    ...
                    $subDiretorios = Storage::disk('local')->directories($diretorioUsuario);
                    if (empty($subDiretorios) && empty(Storage::disk('local')->files($diretorioUsuario))) {
