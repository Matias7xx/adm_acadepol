...
class DashboardController extends Controller
{
    ...
    private function obterMetricasUsuarios($mesAtual, $mesPassado)
    {
        ...
        $percentual = $this->calcularPercentual($usuariosMesAtual, $usuariosMesPassado);
    ...
    private function obterMetricasAlojamento($mesAtual, $mesPassado)
    {
        ...
        $reservasUsuariosMes = Alojamento::where('created_at', '>=', $mesAtual)->count();
        $reservasVisitantesMes = Visitante::where('created_at', '>=', $mesAtual)->count();
        $totalReservasMes = $reservasUsuariosMes + $reservasVisitantesMes;
        
        $reservasPendentesUsuarios = Alojamento::where('status', 'pendente')->count();
        $reservasPendentesVisitantes = Visitante::where('status', 'pendente')->count();
        $totalPendentes = $reservasPendentesUsuarios + $reservasPendentesVisitantes;
        
        $reservasAprovadasUsuarios = Alojamento::where('status', 'aprovada')->count();
        $reservasAprovadasVisitantes = Visitante::where('status', 'aprovada')->count();
        $totalAprovadas = $reservasAprovadasUsuarios + $reservasAprovadasVisitantes;
        
        $reservasRejeitadasUsuarios = Alojamento::where('status', 'rejeitada')->count();
        $reservasRejeitadasVisitantes = Visitante::where('status', 'rejeitada')->count();
        $totalRejeitadas = $reservasRejeitadasUsuarios + $reservasRejeitadasVisitantes;
        
        // Calcular percentual de crescimento
        $reservasMesPassado = Alojamento::whereBetween('created_at', [$mesPassado, $mesAtual])->count() +
                             Visitante::whereBetween('created_at', [$mesPassado, $mesAtual])->count();
        $percentual = $this->calcularPercentual($totalReservasMes, $reservasMesPassado);
    ...
    private function obterMetricasRequerimentos($mesAtual, $mesPassado)
    {
        ...
        $requerimentosMesPassado = Requerimento::whereBetween('created_at', [$mesPassado, $mesAtual])->count();
        $percentual = $this->calcularPercentual($totalMes, $requerimentosMesPassado);
    ...
    private function obterMetricasContatos($mesAtual, $mesPassado)
    {
        ...
        $contatosMesPassado = Contato::whereBetween('created_at', [$mesPassado, $mesAtual])->count();
    ...
    private function obterDadosGrafico()
    {
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $reservasVisitantes = Visitante::whereBetween('created_at', [$inicioMes, $fimMes])->count();
    ...
    public function obterEstatisticasAdicionais()
    {
        ...
        $totalReservas = (Alojamento::whereIn('status', ['aprovada', 'rejeitada'])->count() + 
    ...
    {
        $requerimentosRespondidos = Requerimento::whereIn('status', ['deferido', 'indeferido'])
    ...
    private function calcularOcupacaoAlojamento()
    {
        ...
        $reservasAprovadas = Alojamento::where('status', 'aprovada')
        ...
        return $capacidadeMaxima > 0 ? round(($reservasAprovadas / $capacidadeMaxima) * 100) : 0;
    }
}