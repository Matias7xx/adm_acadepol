    ...
    public function uploadMultipleFiles(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'files.*' => 'file|mimes:pdf,doc,docx,xls,xlsx,ppt,pptx,txt,zip,rar|max:10240',
            ]);
                ...
                try {
                    ...
                    if ($filePath) {
                        $uploadedFiles[] = [
                            ...
                            'fileSize' => $this->formatFileSize($file->getSize()),
                            'mimeType' => $file->getMimeType(),
                            'icon' => $this->getFileIcon($file->getMimeType(), $file->getClientOriginalExtension())
                        ];
                    } else {
            ...
            return response()->json([
                'success' => false,
                'error' => 'Erro de validação: ' . implode(', ', $e->validator->errors()->all())
            ], 422);
    ...
    public function cleanupTempFiles()
    {
        try {
            ...
            foreach ($files as $file) {
                ...
                $fileAge = $now->diffInHours(Carbon::createFromTimestamp($lastModified));
        ...
        try {
            ...
            foreach ($files as $file) {
                ...
                $fileInfo[] = [
                    ...
                    'last_modified' => Carbon::createFromTimestamp($lastModified)->format('d/m/Y H:i:s'),
            ...
            return response()->json([
                ...
                'total_size' => $this->formatFileSize(array_sum(array_map(function($file) {
    ...
    public function deleteTempFile(Request $request)
    {
        try {
            ...
            if (!str_starts_with($filePath, 'noticias/temp/')) {
                return response()->json([
                    'success' => false,
                    'error' => 'Apenas arquivos temporários podem ser removidos por esta rota.'
                ], 400);
            }
            ...
            return response()->json([
                'success' => false,
                'error' => 'Dados inválidos: ' . implode(', ', $e->validator->errors()->all())
            ], 422);
