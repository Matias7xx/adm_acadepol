...
class ContatoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $administradorEmail = config('contato.admin_email', 'matiasnobrega7@gmail.com');
        Mail::to($administradorEmail)->send(new NovoContato($contato));
        
        // Enviar cópia para o e-mail institucional
        $emailInstitucional = config('contato.institutional_email', 'nobregamatias7@gmail.com');
        ...
        return redirect()->route('contato.confirmacao')
            ->with('message', 'Sua mensagem foi enviada com sucesso! Entraremos em contato em breve.');
    }
    ...
    public function alterarStatus(Request $request, Contato $contato)
    {
        ...
        $validator = Validator::make($request->all(), [
            'status' => ['required', Rule::in(['pendente', 'respondido', 'arquivado'])],
        ...
        return redirect()->back()
            ->with('message', "Status da mensagem alterado para {$novoStatus} com sucesso!");
    }
