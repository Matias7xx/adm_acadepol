...
class MatriculaRelatorioController extends Controller
{
    ...
    public function gerarRelatorioPDF(Curso $curso)
    {
        try {
            ...
            $nomeArquivo = 'servidores_matriculados_' . str_replace(' ', '_', $curso->nome) . '_' . now()->format('Y-m-d_H-i-s') . '.pdf';
            ...
            return back()->with('error', 'Erro ao gerar relatório PDF: ' . $e->getMessage());
        }
    }
    ...
    public function gerarRelatorioExcel(Curso $curso)
    {
        try {
            ...
            $sheet->setCellValue('A4', 'Total de Servidores: ' . $matriculas->count());
            $sheet->setCellValue('A5', 'Data de Geração: ' . now()->format('d/m/Y H:i'));
            
            // Aplicar negrito e centralizar as informações do curso
            $sheet->getStyle('A3:A5')->getFont()->setBold(true);
            $sheet->getStyle('A3:A5')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
            ...
            foreach ($cabecalhos as $index => $cabecalho) {
                ...
                $sheet->getStyle($celula)->getBorders()->getAllBorders()
                    ->setBorderStyle(Border::BORDER_THIN);
                $sheet->getStyle($celula)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle($celula)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
            }
            ...
            foreach ($matriculas as $matricula) {
                ...
                $sheet->setCellValue('B' . $linha, $matricula->aluno->matricula ?? 'N/A');
                $sheet->setCellValue('C' . $linha, $matricula->aluno->email ?? 'N/A');
                $sheet->setCellValue('D' . $linha, $matricula->aluno->telefone ?? 'N/A');
                $sheet->setCellValue('E' . $linha, $matricula->created_at->format('d/m/Y'));
                ...
                foreach ($colunas as $coluna) {
                    ...
                    $sheet->getStyle($celula)->getAlignment()
            ...
            $nomeArquivo = 'servidores_matriculados_' . str_replace(' ', '_', $curso->nome) . '_' . now()->format('Y-m-d_H-i-s') . '.xlsx';
