...
class Ocupacao extends Model
{
    ...
    {
        if (!$this->checkin_at) return null;
    ...
    {
        return $this->reservavel_type === 'App\Models\Alojamento' ? 'Usuário' : 'Visitante';
    }
    ...
    {
        if (!$this->reservavel) return 'Reserva não encontrada';

        $nome = $this->reservavel->nome;
        $tipo = $this->tipo_reserva;
        $dataInicio = $this->reservavel->data_inicial ? $this->reservavel->data_inicial->format('d/m/Y') : 'N/A';
        $dataFim = $this->reservavel->data_final ? $this->reservavel->data_final->format('d/m/Y') : 'N/A';
