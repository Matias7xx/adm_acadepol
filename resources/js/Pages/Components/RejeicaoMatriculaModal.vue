...
function confirmarRejeicao() {
  ...
  if (!motivo.value || motivo.value.trim().length < 5) {
    errorMessage.value = 'Por favor, forneça um motivo válido com pelo menos 5 caracteres.';
    return;
  }
      ...
      <DialogPanel class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        ...
        <div class="mt-2">
          <p class="text-sm text-gray-500 mb-4">
            Você está prestes a rejeitar a matrícula de <span class="font-semibold">{{ matriculaInfo?.aluno?.name }}</span> 
            no curso <span class="font-semibold">{{ matriculaInfo?.curso?.nome }}</span>.
          </p>
