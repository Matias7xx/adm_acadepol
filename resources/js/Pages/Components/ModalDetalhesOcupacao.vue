<template>
  ...
  >
    <div class="bg-white rounded-lg p-4 w-full max-w-2xl mx-4 max-h-[85vh] overflow-hidden">
      <!-- Header -->
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold text-gray-900">Detalhes da Ocupação</h3>
        <button 
          @click="$emit('close')"
      ...
      <div class="overflow-y-auto max-h-[calc(85vh-8rem)]">
        ...
        <div v-else class="space-y-4">
          ...
          <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div class="bg-white rounded p-3 border border-gray-200">
                <label class="text-xs font-medium text-gray-500 uppercase">Nome</label>
                <p class="text-gray-900 mt-1 text-sm">{{ ocupacao.hospede_nome || 'Não informado' }}</p>
              </div>
              <div class="bg-white rounded p-3 border border-gray-200">
                <label class="text-xs font-medium text-gray-500 uppercase">Tipo</label>
                <p class="text-gray-900 mt-1 text-sm">{{ ocupacao.hospede_tipo || 'Não informado' }}</p>
              </div>
              <div class="bg-white rounded p-3 border border-gray-200">
                <label class="text-xs font-medium text-gray-500 uppercase">CPF</label>
                <p class="text-gray-900 mt-1 text-sm">{{ formatarCPF(ocupacao.hospede_documento) }}</p>
              </div>
              <div class="bg-white rounded p-3 border border-gray-200">
                <label class="text-xs font-medium text-gray-500 uppercase">Telefone</label>
                <p class="text-gray-900 mt-1 text-sm">{{ ocupacao.hospede_telefone || 'Não informado' }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div class="bg-white rounded p-3 border border-gray-200">
                <label class="text-xs font-medium text-gray-500 uppercase">Check-in</label>
                <p class="text-gray-900 mt-1 text-sm">{{ ocupacao.checkin_at || 'Não informado' }}</p>
              </div>
              <div class="bg-white rounded p-3 border border-gray-200">
                <label class="text-xs font-medium text-gray-500 uppercase">Check-out Previsto</label>
                <p class="text-gray-900 mt-1 text-sm">{{ ocupacao.checkout_previsto || 'Não informado' }}</p>
              </div>
              <div class="bg-white rounded p-3 border border-gray-200">
                <label class="text-xs font-medium text-gray-500 uppercase">Tempo Decorrido</label>
                <p class="text-gray-900 mt-1 text-sm">{{ ocupacao.duracao || 'Calculando...' }}</p>
              </div>
              <div class="bg-white rounded p-3 border border-gray-200">
          ...
          <!-- Observações -->
          <div v-if="ocupacao.observacoes" class="bg-gray-50 rounded-lg p-4 border border-gray-200">
            <h4 class="font-medium text-gray-900 mb-3 text-sm flex items-center">
              <svg class="w-4 h-4 mr-2 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded p-3 border border-gray-200">
              <p class="text-gray-700 text-sm leading-relaxed">{{ ocupacao.observacoes }}</p>
            </div>
          </div>

          <!-- Informações da Reserva -->
          <div v-if="ocupacao.reserva_id" class="bg-gray-50 rounded-lg p-4 border border-gray-200">
            <div class="flex items-center justify-between">
              <div>
                ...
                <div class="mt-2 space-y-1">
                  <p class="text-sm text-gray-700">ID: #{{ ocupacao.reserva_id }}</p>
                  <p class="text-xs text-gray-600">{{ formatarTipoReserva(ocupacao.reserva_type) }}</p>
                </div>
              </div>
              <button 
