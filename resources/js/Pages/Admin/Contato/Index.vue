...
const alterarStatus = (id, novoStatus, mensagemConfirmacao) => {
  ...
  confirmModalMessage.value = mensagemConfirmacao || `Tem certeza que deseja ${mensagens[novoStatus]} esta mensagem?`;
  ...
  confirmModalAction.value = () => {
    ...
    form.patch(route(routeName, id), {
      onSuccess: () => {
        closeConfirmModal();
        toast.success(`Mensagem ${novoStatus === 'arquivado' ? 'arquivada' : 'retornada para pendente'} com sucesso!`);
      ...
      onError: (errors) => {
        closeConfirmModal();
        toast.error(`Erro ao ${mensagens[novoStatus]} mensagem: ${JSON.stringify(errors)}`);
...
const confirmarResposta = () => {
  ...
  if (!mensagemResposta.value || mensagemResposta.value.trim().length < 5) {
    respostaError.value = 'Por favor, forneça uma resposta válida com pelo menos 5 caracteres.';
    return;
  }
  ...
  form.post(route('admin.contato.responder', contatoParaResponder.value.id), {
    onSuccess: () => {
      ...
      // Atualiza o item na lista local
      const index = props.contatos.data.findIndex(c => c.id === contatoParaResponder.value.id);
      if (index !== -1) {
    ...
    onError: (errors) => {
      ...
      if (errors.resposta) {
        ...
        respostaError.value = 'Ocorreu um erro ao processar a resposta. Por favor, tente novamente.';
...
const arquivarContato = (id) => {
  alterarStatus(id, 'arquivado', 'Tem certeza que deseja arquivar esta mensagem? Ela ficará marcada como arquivada no sistema.');
};

const retornarParaPendente = (id) => {
  alterarStatus(id, 'pendente', 'Tem certeza que deseja retornar esta mensagem para pendente?');
};

// Excluir contato
const excluirContato = (id) => {
  confirmModalTitle.value = 'Excluir Mensagem';
  confirmModalMessage.value = 'Tem certeza que deseja excluir permanentemente esta mensagem? Esta ação não pode ser desfeita.';
...
<template>
  <LayoutAuthenticated>
    ...
    <SectionMain>
          ...
          <tbody>
            <tr v-for="contato in props.contatos.data" :key="contato.id">
              ...
              <td data-label="Nome">
                ...
                <small class="text-gray-500 dark:text-gray-400">{{ contato.email }}</small>
              </td>
      ...
      <!-- Paginação -->
      <div class="mt-6" v-if="props.contatos.links && props.contatos.links.length > 3">
        <CardBox>
          <div class="flex items-center justify-between">
            <small>Mostrando {{ props.contatos.from }} a {{ props.contatos.to }} de {{ props.contatos.total }} resultados</small>
            <div class="flex">
              <Link
      ...
      >
        <div class="relative mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white dark:bg-gray-800">
          <div class="mt-3">
            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white text-center">Responder Mensagem</h3>
            <div class="mt-4 px-7 py-3">
              <div class="mb-4 p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
                <h4 class="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-2">Mensagem Original:</h4>
                <p class="text-sm text-gray-800 dark:text-gray-200 mb-1">
                  <span class="font-medium">De:</span> {{ contatoParaResponder?.nome }} ({{ contatoParaResponder?.email }})
                </p>
                <p class="text-sm text-gray-800 dark:text-gray-200 mb-1">
                  <span class="font-medium">Assunto:</span> {{ contatoParaResponder?.assunto }}
                </p>
              ...
              <div class="mb-4">
                <label for="mensagem_resposta" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Sua Resposta *
                </label>
                <textarea
                ...
                ></textarea>
                <p v-if="respostaError" class="mt-1 text-sm text-red-600 dark:text-red-400">
                  {{ respostaError }}
                </p>
              </div>
              
              <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">
                Após enviar, esta mensagem será automaticamente marcada como respondida e um email será enviado para {{ contatoParaResponder?.email }}
              </p>
            </div>
            <div class="flex justify-center gap-4 mt-4">
              <BaseButton 
