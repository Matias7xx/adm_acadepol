...
const anexoUrl = ref(props.requerimento.anexo_url || props.requerimento.documento_url);
...
const motivoRejeicaoExibido = computed(() => {
  return props.requerimento.motivo_rejeicao || 
...
const foiRejeitado = computed(() => {
  return props.requerimento.status === 'rejeitado' || 
...
const alterarStatus = (novoStatus) => {
  ...
  confirmModalAction.value = () => {
    ...
    form.patch(route('admin.requerimentos.alterar-status', props.requerimento.id), {
      onSuccess: () => {
        showConfirmModal.value = false;
        isChangingStatus.value = false;
        toast.success(`Status alterado para ${statusLabels[novoStatus]} com sucesso!`);
...
const confirmarAprovacao = () => {
  if (!respostaAprovacao.value || respostaAprovacao.value.trim().length < 5) {
    aprovacaoError.value = 'Por favor, forneça uma resposta com pelo menos 5 caracteres.';
    return;
  }
  ...
  form.post(route('admin.requerimentos.deferir', props.requerimento.id), {
    ...
    onError: (errors) => {
      ...
      if (errors.resposta) {
        ...
        aprovacaoError.value = 'Ocorreu um erro ao processar a aprovação. Por favor, tente novamente.';
    ...
    <SectionMain>
      ...
      <CardBox class="mb-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            ...
            <div v-if="requerimento.status === 'aprovado' || requerimento.status === 'deferido'" class="flex gap-2">
              <BaseButton
                @click="rejeitarRequerimento"
                :icon="mdiCloseCircle"
                label="Indeferir"
            ...
            <div v-if="requerimento.status === 'rejeitado' || requerimento.status === 'indeferido'" class="flex gap-2">
              <BaseButton
                @click="aprovarRequerimento"
                :icon="mdiCheckCircle"
                label="Aprovar"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <CardBox>
          ...
          <div class="space-y-1">
            ...
            <div class="grid grid-cols-3 gap-2">
              ...
              <span class="col-span-2">{{ requerimento.cargo || 'Não informado' }}</span>
            </div>
            <div v-if="requerimento.orgao" class="grid grid-cols-3 gap-2">
              <span class="font-medium text-gray-600 dark:text-gray-300">Órgão:</span>
            ...
            <div class="grid grid-cols-3 gap-2">
              <span class="font-medium text-gray-600 dark:text-gray-300">E-mail:</span>
        ...
        <CardBox>
          ...
          <div class="space-y-1">
            <div class="grid grid-cols-3 gap-2">
              <span class="font-medium text-gray-600 dark:text-gray-300">Tipo Requerimento:</span>
              <span class="col-span-2">{{ tipoRequerimentoFormatado }}</span>
            </div>
            <div class="grid grid-cols-3 gap-2">
              <span class="font-medium text-gray-600 dark:text-gray-300">Data:</span>
              <span class="col-span-2">{{ formatDateTime(requerimento.created_at) }}</span>
            </div>
            <div v-if="requerimento.protocolo" class="grid grid-cols-3 gap-2">
              <span class="font-medium text-gray-600 dark:text-gray-300">Protocolo:</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <CardBox>
          ...
          <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg text-gray-800 dark:text-gray-200">
            <p class="whitespace-pre-line">{{ requerimento.conteudo }}</p>
          </div>
        </CardBox>
        
        <!-- Documento e Dados Adicionais -->
        <CardBox>
          <h3 class="text-lg font-medium mb-4">Documentação e Dados Adicionais</h3>
          <div class="space-y-4">
            <div v-if="hasAnexo" class="flex items-center justify-between bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
            ...
            <div v-else class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg text-gray-500 dark:text-gray-400 italic flex items-center">
            ...
            <div v-if="requerimento.dados_adicionais" class="pt-4">
              <h4 class="font-medium text-gray-700 dark:text-gray-300 mb-2">Dados Adicionais:</h4>
              <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                <div v-for="(valor, chave) in requerimento.dados_adicionais" :key="chave" class="mb-2">
                  <span class="font-medium">{{ chave }}:</span> {{ valor }}
                </div>
              </div>
            </div>
            ...
            <div v-if="foiRejeitado" class="pt-2">
              ...
              <p class="p-3 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-lg">{{ motivoRejeicaoExibido }}</p>
            </div>
          </div>
        </CardBox>
      </div>
      ...
      <CardBox v-if="requerimento.status === 'aprovado' || requerimento.status === 'deferido'" class="mb-6">
        ...
        <div class="bg-green-50 dark:bg-green-900/30 p-4 rounded-lg text-green-800 dark:text-green-200">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p>{{ requerimento.data_resposta ? formatDateTime(requerimento.data_resposta) : (requerimento.data_aprovacao ? formatDateTime(requerimento.data_aprovacao) : 'Não informado') }}</p>
            </div>
            <div v-if="requerimento.observacao_aprovacao || requerimento.resposta" class="col-span-1 md:col-span-2">
              <div class="font-medium mb-2">Observações/Resposta:</div>
              <p>{{ requerimento.observacao_aprovacao || requerimento.resposta }}</p>
            </div>
          </div>
        </div>
      </CardBox>
      ...
      >
        <div class="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800">
          <div class="mt-3">
            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white text-center">Rejeitar Requerimento</h3>
            <div class="mt-2 px-7 py-3">
              <p class="text-sm text-gray-500 dark:text-gray-300 mb-4">
                Você está prestes a rejeitar o requerimento de <span class="font-semibold">{{ requerimento.nome }}</span>. 
...
>
  <div class="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800">
    <div class="mt-3">
      <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white text-center">Aprovar Requerimento</h3>
      <div class="mt-2 px-7 py-3">
        <p class="text-sm text-gray-500 dark:text-gray-300 mb-4">
          Você está prestes a aprovar o requerimento de <span class="font-semibold">{{ requerimento.nome }}</span>. 
          Por favor, forneça uma resposta:
        </p>
        
        <div class="mb-4">
          <label for="resposta_aprovacao" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Resposta *
          </label>
          <textarea
          ...
          ></textarea>
          <p v-if="aprovacaoError" class="mt-1 text-sm text-red-600 dark:text-red-400">
            {{ aprovacaoError }}
          </p>
        </div>
        
        <div class="mb-4">
          <label for="documento_resposta" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
      ...
      >
        <div class="relative mx-auto p-0 border w-11/12 md:w-4/5 lg:w-3/4 h-5/6 shadow-lg rounded-md bg-white dark:bg-gray-800">
          <div class="flex justify-between items-center p-4 border-b">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">Anexo do Requerimento</h3>
          ...
          <div class="h-5/6 p-2 bg-gray-100 dark:bg-gray-900 flex justify-center overflow-auto">
            <iframe
